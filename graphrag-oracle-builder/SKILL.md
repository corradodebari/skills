...
description: >
  ...
  my docs", "build GraphRAG in Oracle", "generate setup SQL", or "apply setup SQL to graphuser".
---
...
6. **Print** a summary of the graph built
...
- In generated setup SQL scripts, always use `<GRAPH-NAME>_V_` as vertex table prefix and `<GRAPH-NAME>_V_` as edge table prefix, where <GRAPH-NAME> it's provided by user or set to MY_KG by default.
...
`Reply in one line like: codex,generate_and_execute_sql,graphuser,yes,graphrag_example.pdf`
...
Execution gate:
- Continue only after all 5 inputs are valid.
- Echo the selected values in one summary line before starting.
- If mode is `generate_and_execute_sql`, ask for explicit confirmation immediately before running `mcp__sqlcl__run_sqlcl`.
...
3. Run the split pipeline scripts in order: `graph_user.py` (optional), `graph_extract.py`, `graph_store.py`, `chunks_to_langchain_oracle_vs.py` (optional, for LangChain chunk retrieval).
...
- Mandatory: Ask for how many Vertexes and Edges type must be extract, or if the user wants to leave autonomous the process.
...
```text
mcp__sqlcl__run_sqlcl  connection=<graphrag-conn>  script=@temp/graphrag_setup.sql
```

Verify with:

```text
mcp__sqlcl__run_sql  connection=<graphrag-conn>
  SELECT * FROM GRAPH_TABLE (MY_KG MATCH (n) COLUMNS (n.name)) FETCH FIRST 10 ROWS ONLY;
```
...
### Phase 4 - store the chunks in oracle db as Langchain vector store
...
| `chunks_to_langchain_oracle_vs.py` | `--table-name` | `LANGCHAIN_CHUNKS_VS` | Oracle LangChain vector store table |
...
- `graph_store.py` always generates full DDL + DML SQL from `temp/output_schema.json`.
